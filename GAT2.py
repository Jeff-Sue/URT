...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, device, add_skip_connection=True, bias=True,
                 dropout=0.6, layer_type=LayerType.IMP1, log_attention_weights=False):
        super().__init__()
    ...
    def forward(self, in_nodes_features, topology):
        return self.gat_net(in_nodes_features, topology)
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, layer_type, concat=False, activation=nn.ELU(),
...
class GATLayerImp1(GATLayer):
    ...
    """
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=False, activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False):

        super().__init__(num_in_features, num_out_features, num_of_heads, LayerType.IMP1, concat, activation, dropout_prob,
                         add_skip_connection, bias, log_attention_weights)

    def forward(self, in_nodes_features, connectivity_mask, coherence, l):
        #
        # Step 1: Linear Projection + regularization
        #
        # in_nodes_features, connectivity_mask = data  # unpack data
        ...
        #             if j == i + 1:
        #                 in_nodes_features2.append(in_nodes_features[num])
        #             num += 1
        #
        #     in_nodes_features2 = torch.stack(in_nodes_features2).to(in_nodes_features.device)
        #     in_nodes_features2 = self.dropout(in_nodes_features2)
        #     nodes_features_proj2 = torch.matmul(in_nodes_features2.unsqueeze(0), self.proj_param)
        #     nodes_features_proj2 = self.dropout(nodes_features_proj2)
        ...
        #
        if coherence == True:
            all_scores = torch.zeros([nodes_features_proj.size(0), l, l]).to(nodes_features_proj.device)
            num = 0
            for i in range(l - 1):
                for j in range(i + 1, l):
                    scores_target = torch.bmm(nodes_features_proj, self.scoring_fn_target).sum(dim=-1)
                    for k in range(nodes_features_proj.size(0)):
                        all_scores[k, i, j] = scores_target[k, num]
                    num += 1
            all_scores = self.leakyReLU(all_scores)
            all_attention_coefficients = self.softmax(all_scores + connectivity_mask)
        else:
            # Apply the scoring function (* represents element-wise (a.k.a. Hadamard) product)
        ...
        if coherence == True:
            all_attention_coefficients2 = torch.zeros([all_attention_coefficients.size(0), num_of_nodes]).to(all_attention_coefficients.device)
            num = 0
            for i in range(l - 1):
                for j in range(i + 1, l):
                    for k in range(all_attention_coefficients.size(0)):
                        all_attention_coefficients2[k, num] = all_attention_coefficients[k, i, j]
                    num += 1

            out_nodes_features = all_attention_coefficients2.unsqueeze(-1) * nodes_features_proj
